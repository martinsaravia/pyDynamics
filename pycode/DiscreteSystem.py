...
#            self.Stif.append( XM.ELEM[i].stif )
        ...
        # Initialize time vector and derived matrices
        self.InitTime(solu[3][0], solu[3][1], solu[3][2])
        
        # Initialize the Force Vector 
        self.InitForceVector()

    
    def InitTime(self, t0, tf, dt):
        # Form Time Table
        self.CONS['T0'] = t0
        self.CONS['TF'] = tf       
        self.CONS['DT'] = dt
        self.CONS['TIME'] = np.arange(self.CONS['T0'], self.CONS['TF'] + self.CONS['DT'], self.CONS['DT']) 
        self.T = self.CONS['TIME'] 
        ...
        self.Fm_r = np.zeros( (self.CONS['TDIM'], self.nmdof) )       # Magnetic Force via resistance
        self.Fm_z = np.zeros( (self.CONS['TDIM'], self.nmdof) )       # Magnetic Force via impedance
    ...
    def InitForceVector(self):     
        for f in self.Force:
            ...
            if f[1] == 'BASEACCELERATION':
                ...
                if f[2] == 'SWEEP':
                    self.wf = np.zeros(self.CONS['TDIM'])
                    for ts in range(self.CONS['TSTP']-1):
                        for para in f[3]:
                            ...
                            self.wf[0] = w0 / 6.283185
                            self.wf[-1] = wf / 6.283185
...
class REC3(DiscreteSystem):  # Recolector
    ...
    def __init__(self, XM, para, snode, stif, damp, mass, coil, magn,  load, solu, force):
        ...
        self.E['open'] = np.zeros( self.CONS['TDIM'] ) # Total open circuit voltage
        self.dphi = np.zeros( self.CONS['TDIM'] )
        self.cm = np.zeros( self.CONS['TDIM'] )
...
#       Initialization of position and triads (Note that only the FIRST VECTOR of the triad is needed)
        ...
        for i in coil:
            ...
            self.H[i] = np.zeros( (self.CONS['TDIM'], XM.ELEM[i].nseg + 1) ) # Matrix of flux derivative (first index is total of coil)
        for i in magn:
...
#        print('   INFO: The linear resonant frequency of the electrical system is  ', self.we, ' Hz ')
#        if self.Stif[0] >= 0.0:
#            self.wn = (self.Stif[0]/self.Mass[0])**0.5 / (2.0 * np.pi)
#            print('   INFO: The resonant frequency of the mechanical system is  ', self.wn, ' Hz ')
#        else:
#            self.wn = 0.0
#            print('   WARNING: The linear stiffness is negative. Mechanical natural frequency can not be found. ')
    ...
    def MagneticInduction(self, sv):
        ...
        if Magn.flaw is 'NUMERIC':
            ...
            for Coil in self.Coil:
                for i, seg in enumerate(Coil.table): 
                    ...
                    self.H[Coil.name][ts+1, i + 1] = sdphi  # Store the flux derivative of the segment
                    self.H[Coil.name][ts+1, 0] += sdphi       # Store the total flux derivative of the coil
                    self.dphi[ts+1] += sdphi                  # total flux of the harvester
            self.cm[ts+1] = self.dphi[ts+1]**2.0 / self.R      # Magnetic Damping (despreciamos la inductancia)
...
#            self.MagneticInduction(sv)
#        except:
#            print('Warning ---> No magnetic induction done, posible errors in self.MagneticInduction...')
#            pass
        # -- Coulomb Damping Evaluation
        ff = 0.0
        ftol = 1.0E-5  # Friction Tolerance
        dvabs = np.absolute(dv)

             
        if self.CONS['DAMPING'] == 'EXACT':
            fm_flag = 1.0
        if self.CONS['DAMPING'] == 'APPROXIMATE':
            fm_flag = 0.0
        ...
        # -- Damping Matrix
#        damp = self.Damp[0] + ff + (1.0 - fm_flag) * self.cm[ts+1]
        cv = self.Damp[0]                     # viscous damping
        cc = ff                               # dry friction damping
        cm = (1.0 - fm_flag) * self.cm[ts+1]  # elecromagnetic resistance pure damping
        self.KS[0,1] =   1.0
        self.KS[1,0] = - self.Spring[0].kk(du) / self.Mass[0]
        self.KS[1,1] = - (cc + cv + cm) / self.Mass[0]
        self.KS[1,3] =   fm_flag * self.dphi[ts+1] / self.Mass[0]   # Electromagnetic force via impedance damping
        self.KS[2,3] =   1.0
        self.KS[3,1] = - self.dphi[ts+1] / self.L
...
#       Stability of the method
    ...
    def UpdateForceVector(self, ts, sv):       
        ...
        self.Fm_r[ts+1,0] = self.cm[ts+1]   * sv[self.svloc]            # EM Force for pure resistive model
        self.Fm_z[ts+1,0] = self.dphi[ts+1] * sv[self.siloc]    # EM Force for impedance model
        if FM == 'FINITE':
    ...
    def ConvergeStep(self, ts):   #Only for relative formulation    
        ...
        for key, el in self.ELEM.items():
            ...
            self.E['open'][ts]  = self.R * dq
         
        # Update the segment flux derivatives
        self.MagneticInduction(sv)
